...
def question(request):
    if request.method == "POST":
        ...
        request.session['sub_id'] = request.POST.get('sub_id')
        ...
        return render(request, 'question.html', {'data': data1, 'datas': datal})
        ...
        for i in range(1, 11):
            ...
            if j == 1:
                log = questionPaperDetails.objects.create(
                    ...
                    semester=request.session['semester'],slNo=1,qno=i,sub_id=request.session['sub_id'],sem_id=1)
            elif j == 2:
                log = questionPaperDetails.objects.create(
...
def eval1(request):
    if request.method=="POST":
      ...
      #  l=questionPaperDetails.objects.all() 
        ques = generatebarcode.objects.filter(Barcode=12345678)
        print(ques[0].sub_id)
        print(type (ques[0].sub_id))
        print(ques[0].semid)
        print(type (ques[0].semid))
        l=questionPaperDetails.objects.filter(sub_id=ques[0].sub_id,sem_id=ques[0].semid) 
        jdata2 = serializers.serialize("json",l)
        print(l[0])
        #return render(request,"eval1.html",{'json':jdata2},{'dat':l},{'ques':ques})
        return render(request,"eval1.html",{'json':jdata2,'dat':l})
    ...
    if request.method=="POST":
     ...
     #   elif Evaluators=="Evaluator 1":
      ...
      #  l=questionPaperDetails.objects.all() 
        ...
        print(l[0])
        #return render(request,"eval1.html",{'json':jdata2},{'dat':l},{'ques':ques})
        return render(request,"eval3.html",{'json':jdata2,'dat':l})
