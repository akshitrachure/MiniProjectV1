    ...
    if request.method == "POST":
        for i in range(1, 11):
            ...
            if j == 1:
                ...
                print(request.session['sub_id'])
                print(int(request.session['sub_id']))
                print(type (int(request.session['sub_id'])))
                log = questionPaperDetails.objects.create(
                    sqno=j, a=p, branch=request.session['branch'], 
                    semester=request.session['semester'],slNo=1,qno=i,sub_id=int(request.session['sub_id']),sem_id=1)
            elif j == 2:
                log = questionPaperDetails.objects.create(
    ...
    if request.method=="POST":
        ...
        if check:
            ...
            logs = detailed_marks.objects.create(
                studentid=request.session['studentid'],semid=request.session['semid'],sub_id=request.session['subid'],
      ...
      #  l=questionPaperDetails.objects.all() 
        ques = generatebarcode.objects.get(Barcode=12312312)
        print(ques[0].sub_id)
        print(type (ques[0].sub_id))
        print(ques[0].semid)
        print(type (ques[0].semid))
        l=questionPaperDetails.objects.get(sub_id=ques[0].sub_id,sem_id=ques[0].semid) 
        ...
        return render(request,"eval1.html",{'json':jdata2,'dat':l,'g':g,'dat1':dat1,'dat2':dat2})
       ...
       # else:
        ...
        if check:
            ...
            logs = detailed_marks.objects.create(
                ...
                a7=request.session['7a'],b7=request.session['7b'],c7=request.session['7c'],d7=request.session['7d'],total7=request.session['total7'],a8=request.session['8a'],b8=request.session['8b'],c8=request.session['8c'],d8=request.session['8d'],total8=request.session['total8'],max4=request.session['maxtotal4'],
                a9=request.session['9a'],b9=request.session['9b'],c9=request.session['9c'],d9=request.session['9d'],total9=request.session['total9'],a10=request.session['10a'],b10=request.session['10b'],c10=request.session['10c'],d10=request.session['10d'],total10=request.session['total10'],max5=request.session['maxtotal5'],
                FinalTotal=int(request.session['maxtotal5'])+int(request.session['maxtotal4'])+int(request.session['maxtotal3'])+int(request.session['maxtotal2'])+int(request.session['maxtotal1']),Evaluator= int(request.session['Evaluator']))
            logs.save()
            return redirect("/thankyou/")
    else:
      #  l=questionPaperDetails.objects.all() 
        ...
        dat2=l[0].semester
        #return render(request,"eval1.html",{'json':jdata2},{'dat':l},{'ques':ques})
        return render(request,"eval3.html",{'json':jdata2,'dat':l,'g':g,'dat1':dat1,'dat2':dat2})
...
def update(request):
    if request.method=="POST":
        ...
        detailed_marks.objects.get(studentid=request.session['studentid'],semid=request.session['semid'],sub_id=request.session['subid'],Evaluator=request.session['Evaluator']).delete()
